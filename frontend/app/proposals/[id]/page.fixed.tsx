...
export default function ProposalDetailPage() {
  ...
  useEffect(() => {
    const fetchProposal = async () => {
      if (!proposalId) {
        setError(new ApiErrorClient('Proposal ID is missing', 400));
        setLoading(false);
        return;
      }
      setLoading(true);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      <Card>
        ...
        <div className="space-y-2 text-sm text-gray-700 p-6">
      ...
      {isAuthenticated && !isClosed && hasStarted && !hasEnded && (
        <Card>
          <CardHeader>
      ...
      <Card>
        ...
        <div className="space-y-3 p-6">
          ...
          {decryptedTally && (
            <div className="text-xs text-gray-700 break-all">
              <span className="font-semibold">Decrypted tally:</span> {decryptedTally}
            </div>
          )}
        </div>
      </Card>
